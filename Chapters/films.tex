...
El subindice $i$ tiene en cuenta los segmentos del film ($MAA$).
...
\begin{align}
	\begin{aligned}
		...
		& + A\int_0^\infty dz \frac{\left<\phi_{MAA}(z)\right>}{v_{MAA}} \left[f(z)(\ln f(z)+ \beta\mu^0_{MAA^-})\right.\\
		...
		&\qquad \qquad \qquad\qquad \qquad\quad \left.+(1-g_\tau(z))(\ln (1-g_\tau(z))+\beta\mu^0_{\tau d})\right]   \\
		 & +A\int_0^\infty dz \left[\left(\sum_{\gamma } {\rho_\gamma(z) q_\gamma + \sum_\tau{f_\tau(z) \left<\rho_{pro,\tau}(z)\right> q_\tau} +  f(z)\dfrac{\left<\phi_{MAA}(r)\right>}{v_{MAA}}q_{MAA}}\right)\beta\Psi(z) \right. \\ & \qquad \qquad \left.-\frac{1}{2}\beta\epsilon(\nabla\Psi(z))^2 \right]
		\end{aligned}
\end{align}


Nuestro film esta el equilibrio con el bulk de la soluci\'on, la cual posee una composici\'on bien definida (pH, Temperatura, concnentraci\'on salina,  concentraci\'on de prote\'ina). Estas cantidades proveen un potencial qu\'imco el cual debe estar en equilibrio con la soluci\'on cercana a nuestro film. En particular son los potenciales de las especies libres, $\mu_\gamma$, y de la prote\'ina, $\mu_{pro}$.
De esta forma nuestra energ\'ia libre se convierte un un gran potencial termodin\'amico:

\begin{align}
	\begin{aligned}
		\Omega = &F - \sum_\gamma \mu_\gamma N_\gamma -  \mu_{pro} N_{pro} \\
			= &F -\sum_\gamma A\int_0^\infty dz \mu_\gamma \rho_\gamma(z) -  \mu_{pro} N_{pro}  \\
			& \qquad -A\int_0^\infty \mu_{H^+} \left( \sum_\tau\left< \rho_{pro,\tau}(z) \right>g_\tau(z) + (1-f(z))\frac{\left< \phi_{MAA}(z) \right>}{v_{MAA}} \right )
			\end{aligned}
		\label{eq:equil-qco}
\end{align}

\noindent $N_\gamma$ y $ N_{pro}$ son el n'umero total de mol\'eculas. En la \'ultima linea de la expresi\'on de \ref{eq:equil-qco} se tiene en cuenta los protones, y con ello su potencial $\mu_{H^+}$, provenientes de las especies con segmentos titulables: prot\'ina y red polim\'erica respectivamente.


Adicionalmente las condiciones de equilibrio deben satifacer la condición de incompresibilidad del sistema: eq \ref{eq:constraint}.  Esta restricci\'on se incorpora como:

\begin{align}
	\Phi = \Omega +A \int_0^\infty dz\Pi(z){\left[\sum_{\gamma}\rho_\gamma(z) v_\gamma + \sum_\lambda{\left<\rho_{pro,\lambda}(z)\right>v_\lambda} + \sum_i{\left<\phi_i(z)\right>} -1 \right]}
\end{align}


\noindent en donde $\Pi(z)$ es un multiplicador local de Lagrange. Finalmente el potencial obtenido para nuestro sistema se escribe de forma explicita:
...
\begin{align}
	\begin{aligned}
		...
		&\qquad \qquad \qquad\qquad \qquad\quad \left.+(1-g_\tau(z))(\ln (1-g_\tau(z))+\beta\mu^0_{\tau d})\right]   \\
		& +A\int_0^\infty dz \left[\left(\sum_{\gamma } {\rho_\gamma(z) q_\gamma + \sum_\tau{f_\tau(z) \left<\rho_{pro,\tau}(z)\right> q_\tau} +  f(z)\dfrac{\left<\phi_{MAA}(r)\right>}{v_{MAA}}q_{MAA}}\right)\beta\Psi(z) \right. \\ & \qquad \qquad \left.-\frac{1}{2}\beta\epsilon(\nabla\Psi(z))^2 \right] \\
		& +A \int_0^\infty dz\Pi(z){\left[\sum_{\gamma}\rho_\gamma(z) v_\gamma + \sum_\lambda{\left<\rho_{pro,\lambda}(z)\right>v_\lambda} + \sum_i{\left<\phi_i(z)\right>} -1 \right]} \\
		&   -\sum_\gamma A\int_0^\infty dz \left(\beta \mu_\gamma \rho_\gamma(z) - \beta \mu_{pro} \left<\rho_{pro}(z)\right> \right)  \\
		&  -A\int_0^\infty \beta\mu_{H^+} \left( \sum_\tau\left< \rho_{pro,\tau}(z) \right>g_\tau(z) + (1-f(z))\frac{\left< \phi_{MAA}(z) \right>}{v_{MAA}} \right )
	\end{aligned}
\end{align}
 
\textcolor{red}{Todos los subdindices utlizados se encuentran en la tabla.... hacer tabla (?)}

A continuaci\'on se mostrara la optimizaci\'on de este gran potencial respecto de los funcionales de  eq. \ref{eq:funcionales}, ahora adicionando el potencial $\Pi(z)$.

Resultando la optimizaci\'on respecto a la probabilidad de conformaci\'on del film, $P(\alpha)$:
\textcolor{blue}{TRABAJAR CON LA DENSIDAD DEL FILM, TODO ES DE MAA, NO ES NECESARIO USAR VOLUME FRACTION}
