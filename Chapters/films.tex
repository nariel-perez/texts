...
\begin{align}
	...
	& \exp \left[-A\int^\infty\_0 dz^\prime n_\lambda(\theta,z, z^\prime)[v_\lambda\beta\Pi(z^\prime) + q_\lambda \Psi(z^\prime)] \right]
	\end{aligned}
...
\begin{align}
	 \left\{ \begin{array}{lr} \beta\tilde{\mu}^0_{pro} = \beta ^0_{pro}  + n_t\beta( \mu^0_{\tau,d})& : \acute{a}cido \\
	 	 \beta\tilde{\mu}^0_{pro} = \beta ^0_{pro}  + n_t\beta(\mu_{H^+} - \mu^0_{\tau,p}) & : base \end{array} \right. 
\end{align}

\noindent en donde $n_t$ es el n\'umero de segmentos titulables dados por:
\begin{align}
	n_t = A\int_0^\infty dz n_\tau(\theta, z^\prime, z)
\end{align}
