...
\begin{align}
	\begin{aligned}
	\rho_{pro}(\theta, z)v_w = &\tilde{a}_{pro} \exp\left[-A\int_0^\infty dz^\prime n_\tau(\theta,z,z^\prime) \ln f_\tau(z^\prime)\right] \\
	& \exp \left[-A\int^\infty_0 dz^\prime n_\lambda(\theta,z, z^\prime)[v_\lambda\beta\Pi(z^\prime) + q_\lambda \Psi(z^\prime)] \right]
	\end{aligned}
...
\begin{align}
	 \left\{ \begin{array}{lr} \beta\tilde{\mu}^0_{pro} = \beta ^0_{pro}  + cn_\tau\beta( \mu^0_{\tau,d})& : \acute{a}cido \\
	 	 \beta\tilde{\mu}^0_{pro} = \beta ^0_{pro}  + cn_\tau\beta(\mu_{H^+} - \mu^0_{\tau,p}) & : base \end{array} \right. 
\end{align}

\noindent en donde se define el n\'umero de composici\'on, $cn_j$, para un segmento $j$:
\begin{align}
	cn_j = A\int_0^\infty dz n_j(\theta, z^\prime, z)
...
\begin{align}
	\begin{aligned}
		\beta \Phi^b=  & A\sum_{\gamma}{\rho^b_\gamma\left(\ln \left(\rho^b_\gamma v_w\right) -1 + \beta\mu^0_\gamma\right)} \\
		%%%%
		&+ A\sum_{\theta}{\rho^b_{pro}(\theta)\left(\ln \left(\rho^b_{pro}(\theta)\right) -1 + \beta\mu^0_{pro} \right)} \\
		%%%%%%
		& + A \sum_\tau \left<\rho^b_{pro,\tau}\right> \left[g^b_\tau(\ln g^b_\tau+ \beta\mu^0_{\tau p}) +(1-g^b_\tau)(\ln (1-g^b_\tau)+\beta\mu^0_{\tau d})\right]   \\
		%%%%%%%
		& +A \left[\left(\sum_{\gamma } {\rho^b_\gamma q_\gamma + \sum_\tau{f^b_\tau \left<\rho^b_{pro,\tau}\right> q_\tau} }\right)\beta\Psi^b -\frac{1}{2}\beta\epsilon(\nabla\Psi^b)^2 \right] \\ 
		%%%%%%%%
		& +A \beta\Pi^b{\left[\sum_{\gamma}\rho^b_\gamma v_\gamma + \sum_\lambda{\left<\rho^b_{pro,\lambda}\right>v_\lambda}  -1 \right]} \\
		%%%%%%%%%%%
		&   -\sum_\gamma A \left(\beta \mu_\gamma \rho^b_\gamma - \beta \mu_{pro} \left<\rho^b_{pro}\right> \right)   -A \beta\mu_{H^+} \left( \sum_\tau\left< \rho^b_{pro,\tau} \right>g_\tau  \right )
	\end{aligned}
...
\begin{align}
	\begin{aligned}
		\rho_{pro}(\theta)v_w = &\tilde{a}_{pro} \exp\left[-cn_\tau \ln f^b_\tau\right] \\
...
De nuevo podemos observar que nuestros funcionales quedan dependiendo por la presi\'on osm\'otica $\Pi^b$ y el potencial electrost\'atico $\Psi^b$
Sin embargo si consideramo las condiciones de contorno dada en la ec. \ref{eq:si:contorno} para la ecuaci\'on de Poisson, vemos que en el bulk de la soluci\'on $\Psi^b = 0$. 

Esto nos muestra que para nuestro ba\~no t\'ermico la principal inc\'ognicta es la presi\'on osm\'otica: $\Pi^b$.
La cual es posible obtenerla por resoluci\'on num\'erica al sustituir las ecuaciones  \ref{eq:rhofree-bulk} y \ref{eq:rhopro-bulk} en la nueva condici\'on de incompresibilidad dada por:

\begin{align}
	\sum_\gamma \rho^b_\gamma v_\gamma + \sum_\lambda\left< \rho^b_{pro}\right> = 1
	\label{eq:incom-bulk}
\end{align}
